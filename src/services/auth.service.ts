...
export class AuthService {
  ...
  async signUp(
    data: SignUpDto
  ): Promise<{ user: Partial<IUser>; otp: string }> {
  ...
  async verifyOtp(email: string, otp: string): Promise<Partial<IUser>> {
  ...
  async resendOtp(email: string): Promise<{ message: string; otp?: string }> {
    ...
    if (user.otpExpiry && user.otpExpiry > new Date()) {
      const remainingMinutes = Math.ceil(
        (user.otpExpiry.getTime() - Date.now()) / (60 * 1000)
      );
      throw new Error(
        `Please wait ${remainingMinutes} minutes before requesting a new OTP`
      );
    }
  ...
  async linkDevice(userId: string, powerBoxId: string): Promise<Partial<IUser>> {
  ...
  logger.info(`Device linked for user: ${user.email}, PowerBox ID: ${powerBoxId}`);
